defmodule WikiglobeWeb.WikiDataController do
  ...
  def index_events(conn, %{"year" => year, "month" => month} = params) do
    ...
    events = Mongo.find(:mongo, "events", %{date: datetime, title: %{"$ne" => ""}}) |> Enum.to_list()
    render(conn, "index.json", events: events)
  end

  def index_earthquakes(conn, %{"century_start" => century} = params) do
    {century, _} = Integer.parse(century)
    range = 100
    earthquakes = Mongo.find(:mongo, "earthquakes", %{year: %{"$gt" => century, "$lt" => century + range}}) |> Enum.to_list()
    render(conn, "index.json", earthquakes: earthquakes)
  end
